    ...
    function (Controller, JSONModel, Filter, FilterOperator, SearchField, TypeString, UIColumn, mTable, Fragment, MessageBox) {
        ...
        return Controller.extend("cpccomparativeanalysis.controller.cpcView", {
            ...
            onNFAPrint: function () {
                ...
                this.nfaProductClauseTable; // This for the Cash Discount, gst and credit days table 
                //console.log({nfaOtherData, nfaVersionForCS})

                const data = {
                    date: "12-Mar-2023",
                    subject: "Ariba Event Subject",
                    ...
                    dynamicText1: "<p><b>chlorrphyriphos 20% EC is </b></p><ul><li>Both are offers are technically inline with CFCL's requirement.</li><li>Considering very high cost difference b/w offered prices; the cost was negotiated with L1 vendor M/s Key-tech to which vendoroffered a discount of 2% on the quoted rates.</li> <li>Vendor regretted to offer any further discount.</li><li>Material transportation and transit insurance shall be in CFCL's scope.</li><li>GST shall be paid extra as applicable.</li></ul>",
                    quantity: "181 KL (+10%)",
                    packWisePrice: {
                        columns: [{
                            field: "packSize",
                    ...
                    dynamicText2: "<ul><li>Both are offers are technically inline with CFCL's requirement.</li><li>Considering very high cost difference b/w offered prices; the cost was negotiated with L1 vendor M/s Key-tech to which vendoroffered a discount of 2% on the quoted rates.</li> <li>Vendor regretted to offer any further discount.</li><li>Material transportation and transit insurance shall be in CFCL's scope.</li><li>GST shall be paid extra as applicable.</li></ul>",
                    gst: "Extra as applicable, presently @ 18%",
                    creditPeriod: "90 days",
                    cashDiscount: "3% in lieu of credit days on basic invoice...",
                    delivery: "F.O.R at our C&F destinations as per our supply schedule.",
                    paymentPlan: "90% payments of finished goods will be released to CCPL...",
                    pacingQuality: "Quality of the product as well as packing shall be as per the specifications of BIS standard and instecticides Act 1968/ Rules 1971...",
                    productTesting: "Product shall be tested at NABL accredited Lab or any other governace approved & recognized lab...",
                    otherTerms: "As per our Purchase indent to follow.",
                    dynamicText3: "The total purhcase cost of this product of this  product would be around Rs.437 (excluding GST). Mr.Ashish Kumar, Vice President - Sales..."
                };
        
                const getKeys = (data) => {
                    if(Array.isArray(data) && data.length > 0) return Object.keys(data[0])
                    else if(typeof data === "object") return Object.keys(data);
                    return data;
                }
                ...
                const documentContent = /* HTML */ `
                    <div style="display:flex;flex-direction:column;margin: auto;padding: 30px;font-size: 15px;line-height: 24px;    font-family: &quot;Helvetica Neue&quot;, &quot;Helvetica&quot;, Helvetica, Arial, sans-serif;color: #555;${cssVariables}">
                        ...
                        <div style="display: flex; justify-content: space-between; margin: 0px var(--padding-buffer);">
                            <h4 style="flex: 1;">Subject: <span style="font-weight: 200;">${data.subject}</span></h4>
                            <h4>Date: <span style="font-weight: 200;">${data.date}</span></h4>
                        </div>
                        ...
                        <div style="display: flex;flex-direction: column; --border-size: 1px; padding: var(--padding-buffer)">
                            ...
                            <table border="1" style="${tableStyle}">
                                ...
                                <tbody>
                                        ${data.purchaseSalesSummary.listing.reduce((acc, cellData) => acc += `<tr>${data.purchaseSalesSummary.columns.reduce((cellAcc, column) => cellAcc += `<td style="${cellStyle}">${cellData[column.field]}</td>`, "")}</tr>`, "")}
                                </tbody>
                            </table>
                        </div>
        
                        <!--Dynamic Text field 1-->
                        <div style="padding: var(--padding-buffer)">${data.dynamicText1}</div>
                        ...
                        <div style="display: flex;flex-direction: column; --border-size: 1px; padding: var(--padding-buffer)">
                            ...
                            <table border="1" style="${tableStyle}">
                                ...
                                <tbody>
                                        ${nfaMultiVendorPrice.reduce((acc, cellData) => acc += `<tr>${getKeys(nfaMultiVendorPrice).reduce((cellAcc, column) => cellAcc += `<td style="${cellStyle}">${cellData[column] || "-"}</td>`, "")}</tr>`, "")}
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div style="display: flex;flex-direction: column; --border-size: 1px; padding: var(--padding-buffer)">
                            ...
                            <table border="1" style="${tableStyle}">
                                ...
                                <tbody>
                                        ${nfapackingTable.reduce((acc, cellData) => acc += `<tr>${getKeys(nfapackingTable).reduce((cellAcc, column) => cellAcc += `<td style="${cellStyle}">${cellData[column] || "-"}</td>`, "")}</tr>`, "")}
                                </tbody>
                            </table>
                        </div>
                        
                        <!--Dynamic Text field 2-->
                        <div style="padding: var(--padding-buffer)">${data.dynamicText2}</div>
                        
                        <!-- Manual fields-->
                        <div style="display: flex;flex-direction: column;padding: var(--padding-buffer);">
                            <h4 style="${dictGridStyle}"><span style="text-decoration: underline;">GST:</span> <span style="font-weight: 200;">${data.quantity}</span></h4>
        
                            <h4 style="${dictGridStyle}"><span style="text-decoration: underline;">Credit Period:</span> <span style="font-weight: 200;">${data.creditPeriod}</span></h4>
        
                            <h4 style="${dictGridStyle}"><span style="text-decoration: underline;">Cash Discount:</span> <span style="font-weight: 200;">${data.cashDiscount}</span></h4>
        
                            <h4 style="${dictGridStyle}"><span style="text-decoration: underline;">Delivery:</span> <span style="font-weight: 200;">${data.delivery}</span></h4>
        
                            <h4 style="${dictGridStyle}"><span style="text-decoration: underline;">Payment Plan:</span> <span style="font-weight: 200;">${data.paymentPlan}</span></h4>
        
                            <h4 style="${dictGridStyle}"><span style="text-decoration: underline;">Pacing / Quality:</span> <span style="font-weight: 200;">${data.pacingQuality}</span></h4>
                        ...
                        <div style="padding: var(--padding-buffer); margin-top: 2em;">${data.dynamicText3}</div>
                        ...
                        <div style="display: flex;flex-direction: column; --border-size: 1px; padding: var(--padding-buffer)">
                            ...
                            <table border="1" style="${tableStyle}">
                                <thead>
                                    <tr>
                                        ${getKeys(nfaVersionForCS).reduce((acc, curr) => acc += `<th style="${cellStyle}">${curr}</th>`, "")}
                                    </tr>
                                </thead>
                                <tbody>
                                        ${nfaVersionForCS.reduce((acc, cellData) => acc += `<tr>${getKeys(nfaVersionForCS).reduce((cellAcc, column) => cellAcc += `<td style="${cellStyle}">${cellData[column] || "-"}</td>`, "")}</tr>`, "")}
                                </tbody>
                            </table>
                        </div>
                    </div>`;

    ...
    function (Controller, JSONModel, Filter, FilterOperator, SearchField, TypeString, UIColumn, mTable, Fragment, MessageBox) {
        ...
        return Controller.extend("cpccomparativeanalysis.controller.cpcView", {
            ...
            onNFAPrint: function () {
                ...
                console.log({nfaVersionForCS: this.nfaVersionForCS, nfapackingTable: this.nfapackingTable, nfaMultiVendorPrice:this.nfaMultiVendorPrice})
                const data = {
                    date: "12-Mar-2023",
                ...
                const cellStyle = "border: 1px solid #333";
                    ...
                    <div style="display:flex;flex-direction:column;margin: auto;padding: 30px;font-size: 15px;line-height: 24px;    font-family: &quot;Helvetica Neue&quot;, &quot;Helvetica&quot;, Helvetica, Arial, sans-serif;color: #555;${cssVariables}">
                        ...
                        <div style="display: flex;flex-direction: column; --border-size: 1px; padding: var(--padding-buffer)">
                            ...
                            <table border="1" style="border-collapse: collapse;">
                                <thead>
                                    <tr>
                                        ${data.purchaseSalesSummary.columns.reduce((acc, curr) => acc += `<th>${curr.label}</th>`, "")}
                                    </tr>
                                </thead>
                                <tbody>
                                        ${data.purchaseSalesSummary.listing.reduce((acc, cellData) => acc += `<tr>${data.purchaseSalesSummary.columns.reduce((cellAcc, column) => cellAcc += `<td>${cellData[column.field]}</td>`, "")}</tr>`, "")}
                                </tbody>
                            </table>
                        </div>
                        ...
                        <!-- Quantity -->
                        <div style="display: flex; justify-content: space-between; margin: 0px var(--padding-buffer);">
                            <h4 style="flex: 1;"><span style="text-decoration: underline;">Quantity</span>: <span style="font-weight: 200;">${data.quantity}</span></h4>    
                        </div>
                        ...
                        <div style="display: flex;flex-direction: column; --border-size: 1px; padding: var(--padding-buffer)">
                            ...
                            <table border="1" style="border-collapse: collapse;">
                                <thead>
                                    <tr>
                                        ${data.packWisePrice.columns.reduce((acc, curr) => acc += `<th>${curr.label}</th>`, "")}
                                    </tr>
                                </thead>
                                <tbody>
                                        ${data.packWisePrice.listing.reduce((acc, cellData) => acc += `<tr>${data.packWisePrice.columns.reduce((cellAcc, column) => cellAcc += `<td>${cellData[column.field]}</td>`, "")}</tr>`, "")}
                                </tbody>
                            </table>
                        </div>
                ...
                win.document.title = data.Ebeln + " NFA CPC";
